...
class ClassJobCreator:
    ...
    def MainJobCreator(self):
        ...
        sumLoad = 0
        for item in joblist:
            sumLoad = sumLoad + item[1][0][1] * item[1][0][0] * \
                                item[1][0][2]
        print("avg load: ", sumLoad / float(Set.capacity*Set.duration))

        sumTime = 0
        for item in joblist:
            sumTime = sumTime + item[1][1][2]
        print("avg Time: ", sumTime / float(len(joblist)))
    ...
    def realRuntime(core,vmCount):
        if core == 2:
            return (random.choice([29,69,25,44,56,82,34,50,67]))#/vmCount) ,7,5,8,6
        if core ==  4:
            return (random.choice([15,67,58,15,78,15,73,29,49,63,57,34,44,51,28,43,33,53,65]))#/vmCount)#xlarge 8,5,6,4,4,7,7 74,75,60,98,98,11,97
        if core ==  8:
            return (random.choice([60,30,89,56,60,30,85,51,61,96,51,29,8,47,75,79,27,45,66]))#/vmCount)#2xlarge ,8,8,8,5,6,6,7,5,102,106,96,85
        if core== 16:
            return (random.choice([48,21,87,83,43]))#/vmCount) #4xlarge 4,1,,5
        return()
